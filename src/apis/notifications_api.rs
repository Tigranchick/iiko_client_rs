...
/// struct for typed errors of method [`api1_notifications_send_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum Api1NotificationsSendPostError {
...
pub async fn api1_notifications_send_post(
    configuration: &configuration::Configuration,
    authorization: &str,
    timeout: Option<i32>,
    send_notification_request: Option<models::SendNotificationRequest>,
) -> Result<models::CorrelationIdResponse, Error<Api1NotificationsSendPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_authorization = authorization;
    let p_timeout = timeout;
    let p_send_notification_request = send_notification_request;

    let uri_str = format!("{}/api/1/notifications/send", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_send_notification_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<Api1NotificationsSendPostError> = serde_json::from_str(&content).ok();
